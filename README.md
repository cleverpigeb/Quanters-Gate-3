...
`data/provider.py` 定义行情下载所需的结构化协议。`AkShareClient` 和 `LixingerClient` 都实现该协议，`cli.py` 根据配置选择具体实现；缓存和研究流程只接收协议或数据源工厂。因此可以在测试中使用内存数据源，也可以新增离线数据集或备用供应商，而不需要修改因子、收益和回测逻辑。
...
要得到修正后的研究结果，必须先有经过审计的历史成分文件，再使用 AKShare 重复执行 `--build-market-history` 直至完整缓存构建完成，并重新生成研究报告。本地现有报告应视为旧数据口径的历史快照。
